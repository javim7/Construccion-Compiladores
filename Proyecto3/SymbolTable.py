...
class SymbolTable:
    ...
    def display(self):
        table = PrettyTable()
        table.field_names = ["Name", "ID Type", "Data Type", "Value","Inherits","Scope", "Line", "Bytes", "Memory Address"]

        for symbol in self.symbols:
            table.add_row([symbol.name, symbol.id_type, symbol.data_type, symbol.value,symbol.inheritsFrom, symbol.scope, symbol.line, symbol.bytes, symbol.memory_address])
